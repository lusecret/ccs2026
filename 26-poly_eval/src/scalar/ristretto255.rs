...
const R: Scalar = Scalar([
  0xd6ec_3174_8d98_951d, // 15486807595281847581
  0xc6ef_5bf4_737d_cf70, // 14334777244411350896
  0xffff_ffff_ffff_fffe, // 18446744073709551614
  0x0fff_ffff_ffff_ffff, // 1152921504606846975
]);
...
impl Scalar {
  ...
  pub fn random_64<Rng: RngCore + CryptoRng>(rng: &mut Rng) -> Self {
  ...
  pub fn random_128<Rng: RngCore + CryptoRng>(rng: &mut Rng) -> Self {
    let mut limbs = [0u64; 4];
  //  for i in 0..8 {
      limbs[0] = rng.next_u64();
  ...
  pub fn to_u64(&self) -> [u64; 4] {
    let res = Scalar::montgomery_reduce(self.0[0], self.0[1], self.0[2], self.0[3], 0, 0, 0, 0);
    res.0
  }
...
mod tests {
  ...
  #[test]
  fn test_equality() {
    // Scalar重新实现了Eq特性，参见PartialEq的实现
