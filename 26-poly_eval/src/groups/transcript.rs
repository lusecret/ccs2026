use super::super::montgomery;
use super::super::scalar::Scalar;
use super::group::CompressedGroup;
use merlin::Transcript;
use sha3::Shake256;
use digest::Input;
use digest::ExtendableOutput;
use std::io::Read;

pub trait ProofTranscript {
  fn append_protocol_name(&mut self, protocol_name: &'static [u8]);
  fn append_scalar(&mut self, label: &'static [u8], scalar: &Scalar);
  fn append_point(&mut self, label: &'static [u8], point: &CompressedGroup);
  fn challenge_scalar(&mut self, label: &'static [u8]) -> Scalar;
  fn challenge_mont(&mut self, label: &'static [u8]) -> u64;
  fn challenge_by_size(&mut self, label: &[u8], size: usize) -> Scalar;
  fn challenge_vector(&mut self, label: &'static [u8], len: usize) -> Vec<Scalar>;
  fn challenge_vector_mont(&mut self, label: &'static [u8], len: usize) -> Vec<Scalar>;
  fn challenge_vector_by_size(&mut self, label: &'static [u8], len: usize, size: usize) -> Vec<Scalar>;

}

impl ProofTranscript for Transcript {
  fn append_protocol_name(&mut self, protocol_name: &'static [u8]) {
    self.append_message(b"protocol-name", protocol_name);
  }

  fn append_scalar(&mut self, label: &'static [u8], scalar: &Scalar) {
    self.append_message(label, &scalar.to_bytes());
  }

  fn append_point(&mut self, label: &'static [u8], point: &CompressedGroup) {
    self.append_message(label, point.as_bytes());
  }

  fn challenge_scalar(&mut self, label: &'static [u8]) -> Scalar {
    let mut buf = [0u8; 64];
    self.challenge_bytes(label, &mut buf);
    Scalar::from_bytes_wide(&buf)
  }

  fn challenge_mont(&mut self, label: &'static [u8]) -> u64 {
    let mut buf = [0u8; 8];
    self.challenge_bytes(label, &mut buf);

    let v: u64 = buf
      .iter()
      .fold(0u64, |acc, &byte| (acc << 8) | u64::from(byte));

    v % montgomery::get_Q()
  }

  fn challenge_by_size(&mut self, label: &[u8], size: usize) -> Scalar {
    //const IN_LABEL: &[u8] = b"sub circuit system internal label";

    let mut shake = Shake256::default();
    shake.input(label);

    let mut reader = shake.xof_result();
    let mut h_bytes = [0u8; 64];
    reader.read_exact(&mut h_bytes[0..size]).unwrap();
    
    Scalar::from_bytes_wide(&h_bytes)
  }

  

  fn challenge_vector(&mut self, label: &'static [u8], len: usize) -> Vec<Scalar> {
    (0..len)
      .map(|_i| self.challenge_scalar(label))
      .collect::<Vec<Scalar>>()
  }

  fn challenge_vector_mont(&mut self, label: &'static [u8], len: usize) -> Vec<Scalar> {
    (0..len)
      .map(|_i| Scalar::from(self.challenge_mont(label)))
      .collect::<Vec<Scalar>>()
  }

  fn challenge_vector_by_size(&mut self, label: &'static [u8], len: usize, size: usize) -> Vec<Scalar> {
    (0..len)
      .map(|_i| {
        let tmp = self.challenge_scalar(label);
        let  buf_zero = [0u8; 32];
        let mut nonce = [0u8; 32];
        nonce[..size].copy_from_slice(&tmp.to_bytes()[0..size]);
        nonce[size..].copy_from_slice(&buf_zero[size..]);
        Scalar::from_bytes(&nonce).unwrap()
      })
      .collect::<Vec<Scalar>>()
  }
 
}

pub trait AppendToTranscript {
  fn append_to_transcript(&self, label: &'static [u8], transcript: &mut Transcript);
}

impl AppendToTranscript for Scalar {
  fn append_to_transcript(&self, label: &'static [u8], transcript: &mut Transcript) {
    transcript.append_scalar(label, self);
  }
}

impl AppendToTranscript for [Scalar] {
  fn append_to_transcript(&self, label: &'static [u8], transcript: &mut Transcript) {
    transcript.append_message(label, b"begin_append_vector");
    for item in self {
      transcript.append_scalar(label, item);
    }
    transcript.append_message(label, b"end_append_vector");
  }
}

impl AppendToTranscript for CompressedGroup {
  fn append_to_transcript(&self, label: &'static [u8], transcript: &mut Transcript) {
    transcript.append_point(label, self);
  }
}
